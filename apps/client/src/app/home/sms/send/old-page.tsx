...
export default function SendSmsPage() {
  ...
  const [contactGroups, setContactGroups] = useState<ContactGroup[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [creditBalance, setCreditBalance] = useState<{SMS: number; WALLET: number}>({SMS: 0, WALLET: 0});
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [senderIdsResponse, templatesResponse, groupsResponse, creditsResponse] = await Promise.all([
          fetch("/api/sender-ids", {
            headers: { Authorization: `Bearer ${token}` },
          }),
          fetch("/api/templates", {
  ...
  const calculateMessageCost = (text: string): number => {
    ...
    return segments * 0.03; // 0.03 GHS per segment
  };
    ...
    try {
      ...
      if (creditBalance.SMS < totalCost) {
        toast.error("Insufficient SMS credits", {
          description: `You need GHS ${totalCost.toFixed(2)} credits but only have GHS ${creditBalance.SMS.toFixed(2)}`,
          action: {
            label: "Buy Credits",
      ...
      if (insufficientCreditErrors > 0) {
        toast.error(`Insufficient credits for ${insufficientCreditErrors} messages`, {
          id: toastId,
            ...
            <Card>
                    ...
                    <div className="mt-4 pt-2 border-t">
                      ...
                      {creditBalance.SMS < estimatedCost && (
                        <div className="text-sm text-red-500 mt-1">
                          Insufficient credits! Need GHS {(estimatedCost - creditBalance.SMS).toFixed(2)} more
                        </div>
                      )}
                    </div>
                  )}
              ...
              <CardFooter className="flex justify-end">
                <Button 
                  type="submit" 
                  className="w-full md:w-auto" 
                  disabled={isSending || recipients.length === 0 || !senderId || !message || creditBalance.SMS < estimatedCost}
                >
                  {isSending ? (
                    <>
                      <Loader2 className="mr-2 h-4 w-4 animate-spin" />
